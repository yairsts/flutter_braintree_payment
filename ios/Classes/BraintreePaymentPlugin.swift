...
public class BraintreePaymentPlugin: NSObject, FlutterPlugin {
    ...
    private func handleVenmoPayment(_ arguments: Any?) {
        ...
        guard let token = args[Constants.TOKEN_KEY] as? String else {
            flutterResult?(FlutterError(code: Constants.ERROR_KEY, message: "Token is missing", details: nil))
            return
        }
        ...
        debugPrint("Starting Venmo flow with displayName: \(displayName ?? "N/A"), amount: \(amountString ?? "0")")
    ...
    private func handlePayPalPayment(_ arguments: Any?) {
        ...
        guard let token = args[Constants.TOKEN_KEY] as? String,
              let amountString = args[Constants.AMOUNT_KEY] as? String else {
            flutterResult?(FlutterError(code: Constants.ERROR_KEY, message: "Missing required parameters", details: nil))
            return
        }
        ...
        guard let apiClient = BTAPIClient(authorization: token) else {
            flutterResult?(FlutterError(code: Constants.ERROR_KEY, message: "Invalid Braintree token", details: nil))
            return
        }
    ...
    private func handleError(_ error: Error) {
        ...
        flutterResult?(FlutterError(code: Constants.ERROR_KEY, message: error.localizedDescription, details: nil))
    }

    private func handleCancellation() {
        debugPrint("Payment was canceled")
        flutterResult?(FlutterError(code: Constants.CANCELED_KEY, message: "Payment was canceled", details: nil))
    }
    ...
    private func sendSuccessResponse(nonceJson: [String: Any?]) {
        do {
            ...
            if let jsonString = String(data: jsonData, encoding: .utf8) {
                ...
                flutterResult?(FlutterError(code: "ERROR", message: "Failed to convert JSON to string", details: nil))
            }
        } catch {
            flutterResult?(FlutterError(code: "ERROR", message: "Failed to serialize JSON", details: error.localizedDescription))
        }
    }
}
